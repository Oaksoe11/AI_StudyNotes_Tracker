...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {error ? (
        <div className="rounded-md border border-line bg-card p-4 text-sm text-muted shadow-sm">
          Dashboard is ready, but recent activity is still loading. Refresh in a moment if your folders do not appear.
        </div>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <ActionCard href={notes[0] ? `/notes/${notes[0].id}` : "/notes"} icon={<WandSparkles size={18} />} title="View notes" description="Open generated notes when they are ready." />
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Panel title="Folders">
          {folders.length ? (
            ...
            <PanelEmptyText>{error ? "Folders will appear here once the backend responds." : "Create your first course folder."}</PanelEmptyText>
          )}
        </Panel>
        <Panel title="Recent uploads">
          {documents.length ? (
            ...
            <PanelEmptyText>{error ? "Uploads will appear here once recent activity loads." : "Upload a PDF inside a folder."}</PanelEmptyText>
          )}
        </Panel>
        <Panel title="Recent notes">
          {notes.length ? (
            ...
            <PanelEmptyText>{error ? "Notes will appear here once recent activity loads." : "Generate notes from an extracted PDF."}</PanelEmptyText>
          )}
        </Panel>
      </section>
...
function PanelEmptyText({ children }: { children: ReactNode }) {
  // Student note:
  // Empty panels are calmer than showing a big error on the first screen after login.
  return <p className="rounded-md border border-line bg-paper/50 p-3 text-sm text-muted">{children}</p>;
}
